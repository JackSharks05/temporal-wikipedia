...
class WikiDumpParser extends Transform {
  constructor(options = {}) {
    ...
    this.maxRevisions = options.maxRevisions === undefined ? 50 : options.maxRevisions; // limit revisions per article
    this.emitRevisions = options.emitRevisions || false; // optionally let storage own delta encoding
        ...
        switch (name) {
          ...
          case 'revision':
            if (this.currentPage) {
              ...
              if (this.maxRevisions === Infinity || this.currentPage.revisions.length < this.maxRevisions) {
...
async function processDump(dumpPath, options = {}) {
  const {onArticle, onComplete, limit, maxRevisions, emitRevisions} = options;
  return new Promise((resolve, reject) => {
    ...
    const parser = new WikiDumpParser({limit, maxRevisions, emitRevisions});
    const articles = [];
