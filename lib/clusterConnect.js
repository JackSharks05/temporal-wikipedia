...
async function connectToCluster(opts = {}) {
  ...
  if (!nodesFile) {
    ...
    // Broadcast the group to every worker so each one ends up with
    // `globalThis.distribution[gid]` populated. Without this, distributed
    // services scoped to `gid` (e.g. mr's shuffle calling
    // `dist[gid].mem.append`) silently fail on workers because
    // `dist[gid]` is undefined there.
    await new Promise((resolve, reject) => {
